  ...
  const sidebar = (
    <div className="flex h-full flex-col">
      <div className="flex h-18 items-center gap-3 border-b border-sidebar-border px-5">
        ...
        <div>
          <div className="font-display text-base font-semibold">ClinicFlow</div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-sidebar-foreground/50">
      ...
      <div className="border-t border-sidebar-border p-3">
        <div className="rounded-md bg-sidebar-accent px-3 py-3 text-xs leading-relaxed text-sidebar-accent-foreground">
          <b>ClinicFlow AI</b>
          <br />
          <span className="opacity-65">Ambiente protegido da sua clínica</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-18 items-center gap-3 border-b bg-background/95 px-4 backdrop-blur md:px-7">
          ...
          <div className="hidden max-w-sm flex-1 items-center gap-2 rounded-md border bg-muted/45 px-3 md:flex">
          ...
          <div className="ml-auto flex items-center gap-2">
            {orgs.length > 0 && (
              <Select value={orgId} onValueChange={setOrgId}>
                <SelectTrigger className="hidden h-9 w-44 md:flex">
        ...
        <main className="px-4 py-7 md:px-7 lg:px-9">
          <div className="mx-auto max-w-[1480px]">
            <div className="mb-7">
              <p className="mb-1 text-xs font-bold uppercase tracking-[0.13em] text-primary">
                {eyebrow ?? "ClinicFlow AI"}
              </p>
...
function ModuleNavItem({
  ...
  item: { slug: string; label: string; icon: React.ComponentType<{ className?: string }> };
  active: boolean;
}) {
