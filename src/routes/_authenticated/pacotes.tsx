...
const module_ = modules.find((m) => m.slug === "pacotes")!;

export const Route = createFileRoute("/_authenticated/pacotes")({
  head: () => ({
    meta: [
      { title: "Pacotes — ClinicFlow AI" },
      { name: "description", content: module_.copy },
      { property: "og:title", content: "Pacotes — ClinicFlow AI" },
...
const schema = z.object({
  name: z.string().trim().min(2, "Informe o nome do pacote.").max(160),
  description: z.string().trim().max(500).optional(),
  price: z.string().optional(),
  sessions_count: z.string().optional(),
});

const emptyForm = { name: "", description: "", price: "", sessions_count: "" };
...
function Pacotes() {
  ...
  const loadItems = async (organizationId: string) => {
    const { data } = await supabase
      .from("packages")
      .select("id, name, description, price, sessions_count, active")
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("packages").insert({
        ...
        sessions_count: v.sessions_count ? Number(v.sessions_count) : null,
      ...
      toast.error(getErrorMessage(err, "Não foi possível criar o pacote."));
    } finally {
      setBusy(false);
    }
  };
    ...
    <AppShell title="Pacotes">
        ...
        >
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Preencha os dados para criar um novo pacote.</DialogDescription>
            </DialogHeader>
        ...
        {items.map((p) => (
          <article key={p.id} className="rounded-lg border bg-card p-5 shadow-xs">
            ...
            <div className="mt-5 flex items-center justify-between border-t pt-4 text-xs text-muted-foreground">
              <span>
                {p.sessions_count != null ? `${p.sessions_count} sessões` : "Sessões não definidas"}
              </span>
      ...
      {items.length === 0 && (
        <div className="mt-7 rounded-lg border bg-card p-12 text-center text-sm text-muted-foreground">
          Seus pacotes aparecerão aqui, organizados para facilitar o dia a dia da equipe.
        </div>
      )}
    </AppShell>
  );
